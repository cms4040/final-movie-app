...
# 2. 대량의 과거 데이터 생성 파이프라인 (에러 유발 수식 완벽 보정)
@st.cache_data
def load_rich_market_data():
    ...
    # [수정 포인트] 에러가 발생하던 f-string 내부의 깨진 연산 수식을 정상적인 패턴(i % 3 + 1)으로 수정했습니다.
    extended_titles = [np.random.choice(movie_titles) + f" Part {i % 3 + 1}" for i in range(n_samples)]
...
def find_most_similar_movie(df, user_title):
    corpus = df['TITLE'].tolist() + [user_title]
    ...
    max_score = similarity_scores[0][most_similar_idx]
...
# 고도화 로직: 영화명 유사도가 30% 이상인 경우 실제 흥행작 성적을 반영하여 보정
...
if similarity_score > 0.3:
    st.success(f"🔍 **영화명 매칭 확인**: 입력하신 제목이 과거 흥행작 **'{similar_movie['TITLE']}'**과(와) **{similarity_score*100:.1f}%** 유사합니다. 해당 영화의 흥행 브랜드 파워가 가중치로 반영되었습니다.")
else:
    st.info(f"ℹ️ **영화명 매칭 정보**: 과거 데이터 중 제목이 크게 일치하는 핵심 시리즈가 없습니다. (가장 유사한 제목: '{similar_movie['TITLE']}', 유사도: {similarity_score*100:.1f}%)")
