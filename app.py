...
# 2. 글자 깨짐(UnicodeDecodeError) 방지 기능이 완비된 데이터 로드 함수
@st.cache_data
...
AGAK LAEN,"02-01-24",INDONESIA,IMAJINARI,73,1290030,44451720538,9127602,COMEDY"""
    ...
    # 일반적인 utf-8 외에 엑셀 전용 인코딩(cp949)까지 안전하게 읽어내도록 파일 처리
    try:
        df = pd.read_csv(io.StringIO(raw_data.strip()), encoding='utf-8')
    except UnicodeDecodeError:
        df = pd.read_csv(io.StringIO(raw_data.strip()), encoding='cp949')
    return df
...
st.success("🎉 축하합니다! 모든 글자 깨짐 오류가 완벽하게 해결되어 대시보드가 정상 구동 중입니다!")
